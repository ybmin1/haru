...
describe("getFirstMonday", () => {
  ...
  it("returns the correct first Monday when the 1st is not a Monday", () => {
    const result1 = getFirstMonday(d(2026, 2, 1)); //1 Feb is a Sunday -> first Monday is 2 Feb
    expect(result1.getDate()).toBe(2);
    expect(result1.getMonth()).toBe(1); //Feb (0-indexed)
  });
});

describe("getWeeksInMonth", () => {
  it("returns 4 for a typical month", () => {
    expect(getWeeksInMonth(d(2026, 4, 1))).toBe(4);
  });

  it("returns 5 for a month that spans 5 weeks", () => {
    expect(getWeeksInMonth(d(2025, 9, 1))).toBe(5);
  });
});

describe("getWeekRange", () => {
  it("shows a single month range when week stays in the same month", () => {
    expect(getWeekRange(d(2025, 3, 1), 0)).toBe("3-9Mar");
  });

  it("shows both month names when the week crosses a month boundary", () => {
    expect(getWeekRange(d(2025, 3, 1), 4)).toBe("31Mar-6Apr");
  });
});
